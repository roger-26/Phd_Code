...
%                                                                    %
% ********************************************************************
%    Este código tiene la grayscale image autentica, a la cual se le añaden los dos canales adicionales de las TLVQM
%   features normalizados entre 0 y 255. Además, el PCA se calcula a toda
%   la matrix resultante de TLVQM de M*N*2, donde MxN es el número de
%   patches que se calculan, inicialmente de un size de 40x40, alrededor del Bounding Box para que
%   sean cuadrados. Del PCA se toman los coeficientes, no los score.
%   Date: 18 january 2021
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [result] = DLSSV_TLVQMPatches_PCA_LOG10Frames(input, ext, show_img, init_rect, start_frame, end_frame, prep, s_frames)
...
% **********************************************************************
folder_created_Modified_frames = 'PCA_10frames';
mkdir(input, folder_created_Modified_frames);
% *******************************************************************************

%% Calculating the PCA with the first 10 frames
patches_to_save_each_frame = 230; 
disp(['calculating PCA coeffs']);
time_PCA = tic;
for frame_id=start_frame:10
    ...
    %volviendo la matriz 2D para usar PCA normal
    Matrix_Patches_2D= reshape(matrix_patches_58,[2304,58]);
    %asigno los valores aleatorios y guardo esos patches
    r2 = randi(2304,patches_to_save_each_frame,1);
    Ten_Percent_Random_Patches = Matrix_Patches_2D(r2,:);
    Final_Matriz_PCA_10_Frames((((frame_id-1)*patches_to_save_each_frame)+1):frame_id*patches_to_save_each_frame,:) = ...
        Ten_Percent_Random_Patches;
    
end
  %Eliminando las filas con NAN
    Final_Matriz_PCA_10_Frames(:,13)=[];
    Final_Matriz_PCA_10_Frames(:,49:57)=[];
    
    %Calculando el PCA con dos componentes
    [coeff_PCA,score,latent]=pca(Final_Matriz_PCA_10_Frames, 'NumComponents',2);
...
for frame_id = start_frame:end_frame
    ...
    %cambiando el size de 48*48*58 a 2304*58
    Matrix_Patches_2D= reshape(matrix_patches_58,[Number_divisions*Number_divisions,Number_TLVQM_Features]);
    
    %se eliminan las columnas con NaN
    Matrix_Patches_2D(:,13)=[];
    Matrix_Patches_2D(:,49:57)=[];
    
    %Ahora se hace la normalización para que la media quede en cero
    mydata_mean = mean(Matrix_Patches_2D);
    mydata_mean = repmat(mydata_mean,Number_divisions*Number_divisions,1);
    my_data_norm = Matrix_Patches_2D - mydata_mean;
    
    
    reduced_Patch_Matrix = my_data_norm*coeff_PCA;
...
%     [reduced_Patch_Matrix,coeff_PCA3d] = hyperpca(uint8(matrix_patches_58),2);
    ...
    %Aqui los valores quedan entre 0 y 255
    Patch_PCA = reduced_Patch_Matrix_Normalized;
    
    %Aqui hay que volver al original size de 48*48*2
    ...
    %Se asigna según la posición los dos TLVQM channels resultantes del PCA
    for i=1:size(B)
        row_index=rowA(i);
        col_index=colA(i);
       Patch_Image_Original_size(row_index,col_index,:)=B(i,:); 
    end
    
    %J queda con el tamaño de la imagen original, pero con dos canales, que es el TLVQM reducido a dos componentes
    %principales
    J = imresize(Patch_Image_Original_size,[size(I_orig,1) size(I_orig,2)]);
    ...
    %TLVQM luego de PCA(48*48*58 a 48*48*2) e interpolación, y normalización entre 0 y 255
    I2(:,:,1)=grayscale_I;
    I2(:,:,2:3)=J;
