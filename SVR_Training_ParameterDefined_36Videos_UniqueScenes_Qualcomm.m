...
FileName   = 'Data_UniqueScene_Training_36Vid_Qualcomm.mat';
FolderName = ...
...
FileName_MOS   = 'MOS_UniqueScene_Training_36Vid_Qualcomm.mat';
MOS_data_struct_path       = fullfile(FolderName, FileName_MOS);
load(MOS_data_struct_path);   %

MOS_Biased=Training_MOS_Dataset;
RAW_data=data_C3D_All_Videos_path.Training_Data_Dataset;


%Ya se les hizo antes
data_C3D_All_Videos = RAW_data;
...
Number_VidesPerDistortion_TrainingVideosUniqueScene=[6 6 7 5 6 6];
Minimum_Number_Videos_InDistortion= min(Number_VidesPerDistortion_TrainingVideosUniqueScene(:));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%_________________________________
Training_Percentage=80;%__________
Number_Iterations=100;
Test_Same_Training=1;
Use_Fitted_Parameters=1;

%Hyperparameters
Box_Constraint=0.0013755 ;
...
for iteration=1:Number_Iterations
    ...
    for distortion_to_random=1:6
        Number_Videos_Per_Distortion = Number_VidesPerDistortion_TrainingVideosUniqueScene(distortion_to_random);
        rng('shuffle')
    ...
    for current_distortion=1:6
        Number_Videos_Per_Distortion = Number_VidesPerDistortion_TrainingVideosUniqueScene(current_distortion);
        for i=1:Number_Videos_Per_Distortion
            if i <=aux5
