...
if __name__ == '__main__':
    ...
    data_folder = '/media/r/DV_SSD_2TB/ADVSD_Video_frames'

    res_folder = '/home/r/Current_Work_Phd/set_1385_VariacionResolucion/results/FHD_Alpharefine'
    ...
    with open('/home/r/Dropbox/names_test_set.csv', 'rt') as fin:  # context manager
        cin = csv.reader(fin)
        villains = [row for row in cin]  # a list comprehension
        separator1 = '.'
    for name_video_toProcess in villains:
        print(name_video_toProcess)
        name_video = separator1.join(name_video_toProcess).split('.')
        #print(name_video[0])
        separator = ','
        path_video = data_folder + '/' + name_video[0]
        print(path_video)
        start_time = time.time()
        demo(base_path, ar_path, path_video, res_folder)
        elapsed_time.append( time.time() - start_time)
        print(elapsed_time[cont_video])
        cont_video = cont_video + 1

    file = open(
        '/home/r/Dropbox/Javeriana/current_work/tracker_prediction/VariacionResolucion_ImpactoTracker/Results/Time_Results/FHD_AlphaRefinecsv',
        'w+', newline='')
        # writing the data into the file
    with file:
        write = csv.writer(file)
        write.writerow(elapsed_time)
