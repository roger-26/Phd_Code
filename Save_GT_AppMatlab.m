...
%%
f1=cell2mat(GroundTruth_WLPristine);
...
for i=1:size(GroundTruth_WLPristine,1)
    if isempty(GroundTruth_WLPristine{i,:})
        f1(i,:)=[100 100 101 101]; %Para que no sean los mismos que los que se cambian
        %NaN en el GT
    else
                f1(i,:)=GroundTruth_WLPristine{i,:};
%         f(i,:)=GroundTruth_WLPristine(i,:);
    end
end
%%
Name_GroundTruth='groundtruth_rect.txt';
dlmwrite(Name_GroundTruth,f1,'delimiter',',');