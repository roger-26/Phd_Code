...
addpath('C:\Users\rogergo\Documents\liblinear-master\matlab');
...
for iteration=1:Number_Iterations
    ...
    %     parameters_best = sprintf('-C p -s %d',S);
    
    parameters_regresion = sprintf('-s %d -c %1.5f -p %1.5f', S, C, p);
    
    parameters = parameters_regresion;
    aux4=sparse(TrainingData_50FeaturesPerVideo);
%     model1 = train(TrainingMOS_50FeaturesPerVideo, aux4, parameters);
%     [predict_label, accuracy, dec_values] = predict(TestMOS_50FeaturesPerVideo, sparse(TestData_50FeaturesPerVideo), model1);
%     
    
    best = train(TrainingMOS_50FeaturesPerVideo, aux4,'-C -s 11')
    
    
    model = train(TrainingMOS_50FeaturesPerVideo, aux4, sprintf('-c %f -p %f -s 11', best(1),best(2)));
    [predict_label, accuracy, dec_values] = predict(TestMOS_50FeaturesPerVideo, sparse(TestData_50FeaturesPerVideo), model);
    %%
    ...
    R_LibLinear = corrcoef(predict_label,TestMOS_50FeaturesPerVideo);
    Pearson_LibLinear(iteration)=R_LibLinear(1,2)
    
    
    Spearman_LibLinear(iteration)=corr(predict_label,TestMOS_50FeaturesPerVideo,'Type','Spearman')
    iteration;
    fprintf('Iteration %d\n',iteration);
    toc
end
...
fprintf('Pearson = %.4f +- %.4f\nSpearman = %.4f +- %.4f\n',...
    nanmedian(Spearman_LibLinear),nanstd(Spearman_LibLinear),nanmedian(Pearson_LibLinear),nanstd(Pearson_LibLinear));
